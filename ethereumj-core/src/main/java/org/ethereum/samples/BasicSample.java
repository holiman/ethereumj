...
/**
 * The base sample class which creates EthereumJ instance, tracks and report all
 * the stages of starting up like discovering nodes, connecting, syncing
 * 
 * The class can be started as a standalone sample it should just run until full
 * blockchain sync and then just hanging, listening for new blocks and importing
 * them into a DB
 * 
 * This class is a Spring Component which makes it convenient to easily get
 * access (autowire) to all components created within EthereumJ. However almost
 * all this could be done without dealing with the Spring machinery from withing
...
public class BasicSample implements Runnable {
	...
	private static final ConsoleAppender stdoutAppender = (ConsoleAppender) LogManager
	...
	// Spring config class which add this sample class as a bean to the
	...
	private void springInit() {
		...
		// adding the main EthereumJ callback to be notified on different kind
		// of events
	...
	/**
	 * The method tracks step-by-step the instance lifecycle from node discovery
	 * till sync completion. At the end the method onSyncDone() is called which
	 * might be overridden by a sample subclass to start making other things
	...
	protected void waitForDiscovery() throws Exception {
		...
		int bootNodes = config.peerDiscoveryIPList().size() + 1; // +1: home
																	// node
		int cnt = 0;
		while (true) {
	...
	/**
	 * Discovering nodes is only the first step. No we need to find among
	 * discovered nodes those ones which are live, accepting inbound
	 * connections, and has compatible subprotocol versions
	 */
	...
	protected void waitForFirstBlock() throws Exception {
		...
		while (true) {
			...
			if (bestBlock != null
					&& bestBlock.getNumber() > currentBest.getNumber()) {
	...
	private void waitForSync() throws Exception {
		...
		while (true) {
			...
			if (synced) {
				logger.info("[v] Sync complete! The best block: "
						+ bestBlock.getShortDescr());
				syncComplete = true;
				return;
			}
	...
	EthereumListener listener = new EthereumListenerAdapter() {
		...
		@Override
		public void onEthStatusUpdated(Channel channel,
		...
		private String prntHex(byte[] address) {
			if (address == null)
				return "null";
			return Hex.toHexString(address);
		}

		private void prnt(String a, String b) {
			prnt(0, a, b);
		}

		private void prnt(int indent, String a, String b) {
			String i = new String(new char[indent]).replace("\0", "\t");
			System.out.println(i + a + " : " + b);
		}

		private void prntTransactionInfo(Transaction tx) {
			prntTransactionInfo(0, tx);
		}

		private void prntTransactionInfo(int indent, Transaction tx) {
			byte[] to = tx.isContractCreation() ? tx.getContractAddress() : tx
					.getReceiveAddress();
			prnt(indent, "Sender", prntHex(tx.getSender()));
			prnt(indent, "Value", new BigInteger(1, tx.getValue()).toString());

			if (tx.isContractCreation()) {
				prnt(indent, "To [CREATED]", prntHex(tx.getContractAddress()));
			} else {
				prnt(indent, "To", prntHex(tx.getReceiveAddress()));
			}

			prnt(indent, "Data", prntHex(tx.getData()));
			prnt(indent, "Nonce",
					String.valueOf(ByteUtil.byteArrayToLong(tx.getNonce())));
		}

		private void prntTransactionInfo(InternalTransaction itx) {
			int indent = itx.getDeep() + 1;
			prntTransactionInfo(indent, itx);
			prnt(indent, "Depth", String.valueOf(itx.getDeep()));
			prnt(indent, "Index ", String.valueOf(itx.getIndex()));
		}
		private void prntEnd(){
			System.out.println("--------------------------");
		}
		@Override
		public void onTransactionExecuted(TransactionExecutionSummary summary,
			...
			if (summary.isFailed())
				return;
			prntTransactionInfo(tx);

			for (DataWord acct : summary.getDeletedAccounts()) {
				String deleted = Hex.toHexString(acct.getLast20Bytes());
				prnt(0, "Deleted acct", deleted);
			}
			prnt(0, "Fee", summary.getFee().toString());
			prnt(0, "GasUsed", summary.getGasUsed().toString());
			
			prnt(0, "GasPrice", summary.getGasPrice().toString());

			for (InternalTransaction itx : summary.getInternalTransactions()) {
				prntTransactionInfo(itx);
			}
			prntEnd();
			/*
			 * System.out.println("Storage:"); Map <DataWord, DataWord> diffs =
			 * summary.getStorageDiff(); for (Map.Entry<DataWord, DataWord>
			 * entry : diffs.entrySet()) {
			 * System.out.println(prntHex(entry.getKey().getData()) + " : " +
			 * prntHex(entry.getValue().getData())); }
			 */
		}
	};
}
...
Edges: 
	...
	* datasize
	...
	txnum
	itxnum
	type (send, call, callcode, create)
	PTSD (post-tx storage diff) +N or -N bytes
	gascost (in wei)
**/